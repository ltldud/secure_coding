...
*(작성 가이드: 강의 슬라이드 22~23페이지의 "요구사항 도출" 포맷을 그대로 따릅니다 — 이 단계에서는 아직 구체화하지 않고, "~해야 함" 수준의 한 줄짜리 요구사항만 나열합니다. 구체화는 3장에서 합니다.)*
...
- 상품들을 올리고 볼 수 있어야 함
- 플랫폼 사용자들끼리 소통이 가능해야 함
- 악성 유저나 상품을 차단해야 함
- 유저들 간의 송금이 가능해야 함 (직접 설계)
- 상품을 검색할 수 있어야 함 (직접 설계)
- 관리자가 플랫폼의 모든 요소를 관리할 수 있어야 함 (직접 설계)

### 2.2 비기능적 요구사항

*(작성 가이드: 슬라이드는 "보안/디자인/서버속도"를 예시로 들었는데, 본인 프로젝트에서 실제로 신경 쓴 비기능 요소를 적으면 됨)*
...
*(작성 가이드: 슬라이드 24페이지 — "요구사항을 실제로 어떻게 구현할 것인지 구체화하고 그에 따라 시스템을 디자인하는 것". 25~28페이지 순서(요구사항 구체화 → 기능 재분류 → 웹페이지 설계 → 데이터베이스 설계)를 그대로 따랐습니다.)*

### 3.1 요구사항 구체화

*(작성 가이드: 슬라이드 25페이지 포맷 — 2.1의 요구사항 한 줄 한 줄을 실제로 어떻게 만족시킬지 구체적인 규칙으로 풀어씀. 아래는 실제 구현 코드 기준.)*

- 사람들이 플랫폼에 가입할 수 있어야 함
  - 회원가입 페이지가 있어야 함 — 아이디는 영문/숫자/밑줄 3~20자, 비밀번호는 8~64자(영문+숫자 각 1자 이상)
  - 로그인 페이지가 있어야 함 — 5회 연속 실패 시 15분간 계정 잠금
  - 아이디는 중복되지 않아야 함
  - 마이페이지 기능이 필요함 (소개글, 비밀번호 변경 — 변경 시 현재 비밀번호 재확인)
  - 다른 사용자의 공개 프로필을 확인할 수 있어야 함
  - 유저 정보는 데이터베이스로 관리해야 함
- 상품들을 올리고 볼 수 있어야 함
  - 상품 등록이 가능한 페이지를 만들어야 함 — 상품명 1~100자, 설명 1~2000자, 가격 0~10억 범위 검증
  - 내가 등록한 상품을 확인 및 관리(수정·삭제)할 수 있어야 하며, 이는 소유자 본인만 가능해야 함
  - 등록된 상품은 누구나 볼 수 있어야 함 — 단, 차단된 상품은 노출되지 않아야 함
  - 상품을 볼 때는 이름만 보여주고, 클릭하면 상세 페이지를 볼 수 있어야 함
  - 상품 정보는 데이터베이스로 관리해야 함
- 플랫폼 사용자들끼리 소통이 가능해야 함
  - 전체 유저가 소통할 수 있는 채팅이 있어야 함
  - 각각의 유저가 1대1로 소통할 수 있는 채팅이 있어야 함
  - 인증되지 않은 사용자는 채팅 연결 자체가 거부되어야 함
  - 메시지는 길이 제한(500자)과 도배 방지를 위한 속도 제한(10초당 8건)이 있어야 함
- 악성 유저나 상품을 차단해야 함
  - 불량 상품 혹은 사용자를 신고할 수 있어야 함
  - 신고 사유를 신고 시 작성하게 만들어야 함
  - 동일한 대상은 같은 사람이 중복 신고할 수 없어야 함
  - 일정 횟수(5회) 이상 신고된 상품은 자동으로 차단되어야 함
  - 일정 횟수(5회) 이상 신고된 유저는 휴면 계정으로 자동 전환되어야 함
  - 관리자가 신고를 수동으로 검토·기각할 수 있어야 함
- 유저들 간의 송금이 가능해야 함 (직접 설계)
  - 로그인한 사용자만 송금할 수 있어야 함
  - 자기 자신에게는 송금할 수 없어야 함
  - 잔액이 부족하면 송금이 거부되어야 함
  - 정지(휴면)된 사용자에게는 송금할 수 없어야 함
  - 상품 구매는 "송금 + 상품 상태 변경"이 하나의 트랜잭션으로 처리되어야 함
  - 모든 송금 내역은 감사 가능하도록 데이터베이스에 기록되어야 함
- 상품을 검색할 수 있어야 함 (직접 설계)
  - 로그인한 사용자는 상품명/설명으로 상품을 검색할 수 있어야 함
  - 차단된 상품은 검색 결과에 노출되지 않아야 함
  - 검색어에 의한 SQL Injection이 불가능해야 함
- 관리자가 플랫폼의 모든 요소를 관리할 수 있어야 함 (직접 설계)
  - 관리자 계정은 소스코드에 하드코딩되지 않고 별도 절차(환경변수)로 생성되어야 함
  - 관리자는 회원(정지/해제), 상품(차단/해제/삭제), 신고(검토/기각), 거래 내역을 조회·관리할 수 있어야 함
  - 관리자가 아닌 사용자는 관리자 페이지에 접근할 수 없어야 함(403)
  - 관리자의 주요 조치는 감사 로그로 남아야 함

### 3.2 기능 설계

*(작성 가이드: 슬라이드 26페이지 포맷 — 위에서 구체화한 요구사항을 다시 "기능" 단위로 재분류. 실제 코드의 블루프린트 구조와 대응됨.)*
...
- 악성 유저 필터링
  - 불량 유저 및 상품 신고 기능
  - 불량 상품 차단 기능 (자동 임계치 + 관리자 수동)
  - 불량 유저 휴면 기능 (자동 임계치 + 관리자 수동)
- 송금 기능 (직접 설계)
  - 유저 간 송금 기능
  - 상품 구매(송금 연동) 기능
  - 거래 내역 조회 기능
