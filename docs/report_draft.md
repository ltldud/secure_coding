...
*(작성 가이드: 강의 슬라이드의 "시스템 설계" 포맷 — 기능을 큰 분류별 불릿으로 나열 → 웹페이지 설계 → 데이터베이스 설계 순서 — 을 그대로 따르되, 내용은 실제 구현 코드 기준으로 채웠습니다. 강의에서 직접 설계하라고 한 **송금/검색/관리자**는 별도 분류로 추가했습니다.)*

### 3.1 기능 설계

- 유저 관리
  - 회원가입 기능 (아이디 중복 불가, 비밀번호 정책 검증)
  - 로그인 기능 (5회 실패 시 계정 잠금)
  - 사용자 조회 기능 (공개 프로필)
  - 마이페이지 기능 (소개글 및 비밀번호 업데이트)
- 상품 관리
  - 상품 등록 기능
  - 등록된 상품 관리 기능 (수정/삭제는 소유자만 가능)
  - 상품 조회 기능 (목록에는 이름만 노출)
  - 상품 상세 페이지 기능 (클릭 시 상세 정보 확인)
- 유저 소통 기능
  - 실시간 전체 채팅 기능
  - 1대1 채팅 기능
- 악성 유저 필터링
  - 불량 유저 및 상품 신고 기능 (신고 사유 필수)
  - 일정 횟수(5회) 이상 신고된 상품은 자동 차단
  - 일정 횟수(5회) 이상 신고된 유저는 휴면(정지) 계정으로 전환
- 송금 기능 (직접 설계)
  - 로그인한 유저 간 송금
  - 자기 자신 송금 금지, 잔액 부족 시 거부, 정지 유저에게 송금 금지
  - 상품 구매 시 송금 + 상품 상태 변경이 하나의 트랜잭션으로 처리
  - 모든 송금/구매 내역 조회 기능
- 검색 기능 (직접 설계)
  - 로그인한 유저가 상품명/설명으로 상품 검색
  - 차단된 상품은 검색 결과에서 제외
- 관리자 기능 (직접 설계)
  - 회원(정지/해제), 상품(차단/해제/삭제), 신고(기각), 거래 내역 통합 관리
  - 관리자 계정은 코드에 하드코딩하지 않고 환경변수 기반 시드 스크립트로 생성
