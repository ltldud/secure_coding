...
- **회원 관리**: 회원가입/로그인/로그아웃, 마이페이지(소개글·비밀번호 변경), 공개 프로필 조회
- **상품 관리**: 등록/수정/삭제(소유자만), 목록 조회, 검색(제목/설명), 상세 페이지
- **채팅**: 전체 채팅방(소켓 기반), 상품 판매자와의 1:1 채팅
...
## 알려진 한계 (운영 배포 시 추가 필요)

- 로컬 개발 서버(Werkzeug)로 실행되므로 실제 운영 시 gunicorn+eventlet/gevent 등 프로덕션 WSGI 서버와 리버스 프록시(Nginx 등)로 TLS 종단이 필요합니다.
- 송금 동시성 처리는 단일 프로세스 SQLite 기준으로 검증되었으며, 다중 프로세스/실사용 환경에서는 RDBMS 레벨 락(`SELECT ... FOR UPDATE`)이 추가로 필요합니다.
- Rate limiting/채팅 rate limit은 인메모리 저장소를 사용하므로, 다중 프로세스 배포 시 Redis 등 공유 저장소로 교체가 필요합니다.
